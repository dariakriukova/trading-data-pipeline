...
class IntTestXetraETLMethods(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.s3_bucket_name_src = "xetra-int-test-src-daria"
        self.s3_bucket_name_trg = "xetra-int-test-trg-daria"
        self.meta_key = "meta_file.csv"
        # Creating the source and target bucket on the mocked s3
        self.s3 = boto3.resource(service_name="s3", endpoint_url=self.s3_endpoint_url)
        self.src_bucket = S3BucketConnector(
            bucket=self.s3_bucket_name_src, endpoint_url=self.s3_endpoint_url
        )
        self.trg_bucket = S3BucketConnector(
            bucket=self.s3_bucket_name_trg, endpoint_url=self.s3_endpoint_url
        )
        ...
        self.src_bucket.write_df_to_s3(
            self.df_src.loc[0:0],
    ...
    def tearDown(self):
        for key in self.src_bucket._bucket.objects.all():
            key.delete()
        for key in self.trg_bucket._bucket.objects.all():
            key.delete()
    ...
    def test_int_etl_report1_no_metafile(self):
        ...
        trg_file = self.trg_bucket.list_files_in_prefix(self.target_config.trg_key)[0]
        data = self.trg_bucket._bucket.Object(key=trg_file).get().get("Body").read()
        out_buffer = BytesIO(data)
        df_result = pd.read_parquet(out_buffer)
        self.assertTrue(df_exp.equals(df_result))
        meta_file = self.trg_bucket.list_files_in_prefix(self.meta_key)[0]
        df_meta_result = self.trg_bucket.read_csv_to_df(meta_file)
